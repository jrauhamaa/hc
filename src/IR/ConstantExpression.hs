module IR.ConstantExpression where

{- This module contains functions for evaluating a constant expression -}
...
{- Convert double to int or return an error in case the double value isn't a
   whole number. -}
...
{- The main function. Returns a double. In case of integer expression, the
   double value should be a whole number -}
...
evaluateLogicalOrExpression
     (CLogicalOrExpression
       andExpr
       ParseItem { parseItem = CLogicalOrExpression'Empty }) =
  evaluateLogicalAndExpression (parseItem andExpr)
...
evaluateLogicalAndExpression
     (CLogicalAndExpression
       orExpr
       ParseItem { parseItem = CLogicalAndExpression'Empty }) =
  evaluateInclusiveOrExpression (parseItem orExpr)
...
evaluateLogicalAndExpression' ::
     Double -> CLogicalAndExpression' -> EvaluationResult
evaluateLogicalAndExpression' _ CLogicalAndExpression'Empty = return 1
...
evaluateInclusiveOrExpression' ::
     Double -> CInclusiveOrExpression' -> EvaluationResult
...
evaluateExclusiveOrExpression' ::
     Double -> CExclusiveOrExpression' -> EvaluationResult
...
evaluateEqualityExpression
     (CEqualityExpression
       relExpr
       ParseItem { parseItem = CEqualityExpression'Empty }) =
  evaluateRelationalExpression (parseItem relExpr)
...
evaluateEqualityExpression' ::
     Double -> CEqualityExpression' -> EvaluationResult
evaluateEqualityExpression' _ CEqualityExpression'Empty = return 1
...
-- Return 1 if ture or 0 if false
evaluateRelationalExpression :: CRelationalExpression -> EvaluationResult
evaluateRelationalExpression
     (CRelationalExpression
       shiftExpr
       ParseItem { parseItem = CRelationalExpression'Empty }) =
  evaluateShiftExpression (parseItem shiftExpr)
...
evaluateRelationalExpression' ::
     Double -> CRelationalExpression' -> EvaluationResult
evaluateRelationalExpression' _ CRelationalExpression'Empty = return 1
evaluateRelationalExpression'
     x (CRelationalExpression'LT shiftExpr relExpr') = do
  x' <- evaluateShiftExpression (parseItem shiftExpr)
  if x < x'
    then evaluateRelationalExpression' x' (parseItem relExpr')
    else return 0
evaluateRelationalExpression'
     x (CRelationalExpression'LTE shiftExpr relExpr') = do
  x' <- evaluateShiftExpression (parseItem shiftExpr)
  if x <= x'
    then evaluateRelationalExpression' x' (parseItem relExpr')
    else return 0
evaluateRelationalExpression'
     x (CRelationalExpression'GT shiftExpr relExpr') = do
  x' <- evaluateShiftExpression (parseItem shiftExpr)
  if x > x'
    then evaluateRelationalExpression' x' (parseItem relExpr')
    else return 0
evaluateRelationalExpression'
     x (CRelationalExpression'GTE shiftExpr relExpr') = do
  x' <- evaluateShiftExpression (parseItem shiftExpr)
  if x >= x'
    then evaluateRelationalExpression' x' (parseItem relExpr')
    else return 0
...
evaluateAdditiveExpression' ::
     Double -> CAdditiveExpression' -> EvaluationResult
...
evaluateMultiplicativeExpression ::
     CMultiplicativeExpression -> EvaluationResult
evaluateMultiplicativeExpression
     (CMultiplicativeExpression castExpr multExpr') = do
  x <- evaluateCastExpression (parseItem castExpr)
  evaluateMultiplicativeExpression' x (parseItem multExpr')

evaluateMultiplicativeExpression' ::
     Double -> CMultiplicativeExpression' -> EvaluationResult
evaluateMultiplicativeExpression' x CMultiplicativeExpression'Empty = return x
evaluateMultiplicativeExpression'
     x (CMultiplicativeExpression'Mul castExpr multExpr') = do
  x' <- evaluateCastExpression (parseItem castExpr)
  evaluateMultiplicativeExpression' (x * x') (parseItem multExpr')
evaluateMultiplicativeExpression'
     x (CMultiplicativeExpression'Div castExpr multExpr') = do
  x' <- evaluateCastExpression (parseItem castExpr)
  -- this might cause problems with integers
  -- if result is expected to be rounded down
  evaluateMultiplicativeExpression' (x / x') (parseItem multExpr')
evaluateMultiplicativeExpression'
     x (CMultiplicativeExpression'Mod castExpr multExpr') = do
  x' <- evaluateCastExpression (parseItem castExpr)
  ix <- doubleToInteger (parseLoc castExpr) x
  ix' <- doubleToInteger (parseLoc castExpr) x'
  evaluateMultiplicativeExpression'
    (fromIntegral (ix `mod` ix'))
    (parseItem multExpr')
...
evaluateUnaryExpression (CUnaryExpressionInc i) =
  Left . SyntaxError (parseLoc i) $
       "increment operator in a constant expression"
evaluateUnaryExpression (CUnaryExpressionDec i) =
  Left . SyntaxError (parseLoc i) $
       "decrement operator in a constant expression"
...
evaluatePostfixExpression' ::
     Location -> Double -> CPostfixExpression' -> EvaluationResult
...
evaluateConstant (CConstantChar c) =
  return . fromIntegral . ord . parseItem $ c
